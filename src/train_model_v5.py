...
Changes vs v4
─────────────
* Uses make_point_features_v5 (longer lags/leads, larger windows, neighbour features)
* LightGBM: 3 000 estimators, lr=0.02, num_leaves=95, early stopping on validation
* CatBoost: early stopping on validation
* Default processed dir: data/processed_v5
* Metrics written to reports/model_results_v5.csv
"""
...
from make_point_features_v5 import FEATURE_COLUMNS
...
def build_model(model_type: str, random_state: int) -> Any:
    ...
    if model_type == "lightgbm":
        ...
        return lgb.LGBMRegressor(
            ...
            learning_rate=0.025,        # optimal: lr=0.030 was unstable (RMSE 16.8213)
            num_leaves=63,              # same as v4 (95 caused overfitting)
            max_depth=-1,
...
def default_output_path(
    ...
    return base_dir / f"{model_type}_v5_{rows_label}.{suffix}"


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--model-type", choices=MODEL_TYPES, default="lightgbm")
    parser.add_argument("--processed-dir", type=Path, default=Path("data/processed_v5"))
    ...
    parser.add_argument(
        "--metrics-output", type=Path, default=Path("reports/model_results_v5.csv")
    )
    ...
    model = train_model(train, args.model_type, args.random_state, valid=valid)
