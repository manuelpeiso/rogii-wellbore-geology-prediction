...
from make_point_features_v4 import FEATURE_COLUMNS

# These wells appear in raw/train/ with full TVT but are also the Kaggle test wells.
# Excluding them prevents data leakage: the model would otherwise memorise their TVT.
...
def require_package(package_name: str, install_name: str | None = None) -> Any:
    try:
        ...
        raise ImportError(
            f"Model requires optional package '{package_name}'. Install it with: "
...
def choose_validation_wells(train_path: Path, valid_size: float, random_state: int) -> set[str]:
    wells = pd.read_csv(train_path, usecols=["well_id"])["well_id"]
    # Exclude test wells before choosing validation split
    unique_wells = pd.Series([w for w in wells.unique() if w not in TEST_WELL_IDS])
...
def load_train_data(
    ...
    for chunk in pd.read_csv(train_path, usecols=usecols, chunksize=chunksize):
        # Drop test wells from every chunk before anything else
...
def train_model(train: pd.DataFrame, model_type: str, random_state: int) -> Any:
...
def append_metrics(metrics_path: Path, model_type: str, max_train_rows: int, metrics: dict[str, float]) -> None:
...
def default_output_path(base_dir: Path, model_type: str, max_train_rows: int, suffix: str) -> Path:
    rows_label = f"{max_train_rows // 1000}k" if max_train_rows % 1000 == 0 else str(max_train_rows)
    return base_dir / f"{model_type}_v4_{rows_label}.{suffix}"


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--model-type", choices=MODEL_TYPES, default="lightgbm")
    parser.add_argument("--processed-dir", type=Path, default=Path("data/processed_v4"))
    ...
    parser.add_argument("--metrics-output", type=Path, default=Path("reports/model_results_v4.csv"))
    ...
    print(f"Test wells excluded from training: {sorted(TEST_WELL_IDS)}")
    ...
    model = train_model(train, args.model_type, args.random_state)
    ...
    joblib.dump(
        {"model": model, "model_type": args.model_type, "features": FEATURE_COLUMNS, "metrics": metrics},
        model_output,
    )
