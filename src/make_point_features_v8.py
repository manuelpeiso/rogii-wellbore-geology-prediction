...
Changes vs v5
─────────────
* Adds pre-PS template matching alongside typewell matching.
* Adds similarity rates for pre-PS and typewell matches.
* Keeps the v5 feature set as the base.
"""
...
FEATURE_COLUMNS = [
    ...
    "pre_ps_similarity_rate",
    "typewell_similarity_rate",
...
def mse_to_similarity(mse: np.ndarray) -> np.ndarray:
    return np.where(np.isfinite(mse), 1.0 / (1.0 + mse), np.nan)
...
def build_well_features(
    ...
    first_tvt = float(known_tvt.iloc[0]) if len(known_tvt) else np.nan
    last_tvt = float(known_tvt.iloc[-1]) if len(known_tvt) else np.nan
    # Narrow well proxy based on observed TVT_input spread only (no target leakage).
    ...
    # ── Pre-PS template matching: same 5-point window, but using lateral pre-PS rows ──
    pre_ps_mask = ~tvt_input_missing
    pre_ps_gr = gr[pre_ps_mask].to_numpy(dtype=float)
    pre_ps_tvt = horizontal["TVT_input"][pre_ps_mask].to_numpy(dtype=float)
    if len(pre_ps_gr) > 4:
        pre_ps_as_typewell = pd.DataFrame({"GR": pre_ps_gr, "TVT": pre_ps_tvt})
        _, nearest_pre_ps_window_mse, _ = (
            nearest_typewell_by_gr_window(pre_ps_as_typewell, gr_window_matrix, half=2)
        )
    else:
        nearest_pre_ps_window_mse = np.full(len(horizontal), np.nan)

    pre_ps_similarity_rate = mse_to_similarity(nearest_pre_ps_window_mse)
    typewell_similarity_rate = mse_to_similarity(nearest_typewell_window_mse)
    ...
    features = pd.DataFrame(
        {
            ...
            "pre_ps_similarity_rate": pre_ps_similarity_rate,
            "typewell_similarity_rate": typewell_similarity_rate,
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/processed_v8"))
