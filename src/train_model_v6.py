...
Changes vs v5
─────────────
* Uses make_point_features_v6 (v5 base + pre-PS/typewell similarity features)
* LightGBM: 3 000 estimators, lr=0.02, num_leaves=95, early stopping on validation
* CatBoost: early stopping on validation
* Default processed dir: data/processed_v6
* Metrics written to reports/model_results_v6.csv
"""
...
from make_point_features_v6 import FEATURE_COLUMNS
...
def default_output_path(
    ...
    return base_dir / f"{model_type}_v6_{rows_label}.{suffix}"


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--model-type", choices=MODEL_TYPES, default="lightgbm")
    parser.add_argument("--processed-dir", type=Path, default=Path("data/processed_v6"))
    ...
    parser.add_argument(
        "--metrics-output", type=Path, default=Path("reports/model_results_v6.csv")
    )
